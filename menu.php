...
<body >
	...
	<div class="menuinner">
		<nav class="navbar navbar-dark bg-primary"><a class="navbar-brand" 
		 >Menu</a>
		<a class="btn btn-outline-light my-2 my-sm-0" href="cart.php">Cart</a>
		</nav>
			...
			$row= mysqli_num_rows($data);
				...
				for($i=1 ; $i <= $row ; $i++){
					$query = "SELECT * FROM `menu` WHERE id=$i"; 
					$data = mysqli_query($conn, $query);
					...
					if ($result['type'] == Veg) {
						...
						echo "<p style='font-size: 25px'><b id='Name'>";
						echo  $result['Name'];
						echo "</b></p><br>";
						echo "<b>Price :<b id='Price'>";
						echo $result['Price'];
						echo "</b></b>";
						echo "<input type='checkbox' style='float:right' name='ordered_item[]' value='";
						echo ''.$result['id'].'x'.$i.'';
						echo "'>";
						echo "</div >";
					}	
